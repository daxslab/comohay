# Generated by Django 3.0.5 on 2020-04-22 21:55

from django.db import migrations

from ads.models.municipality import Municipality
from ads.models.province import Province

municipality_codes = {
    '25.03': 'Martí',
    '25.02': 'Cárdenas',
    '23.06': 'Habana del Este',
    '24.04': 'Santa Cruz del Norte',
    '23.03': 'Centro Habana',
    '23.05': 'Regla',
    '26.03': 'Sagua La Grande',
    '23.02': 'Plaza de la Revolución',
    '23.04': 'La Habana Vieja',
    '23.10': 'Cerro',
    '23.09': 'Diez de Octubre',
    '23.07': 'Guanabacoa',
    '23.08': 'San Miguel del Padrón',
    '23.01': 'Playa',
    '23.14': 'Arroyo Naranjo',
    '23.13': 'Boyeros',
    '23.11': 'Marianao',
    '24.02': 'San José de las Lajas',
    '24.03': 'Jaruco',
    '23.15': 'Cotorro',
    '23.12': 'La Lisa',
    '22.05': 'Bauta',
    '25.01': 'Matanzas',
    '22.04': 'Caimito',
    '24.05': 'Madruga',
    '25.08': 'Limonar',
    '26.01': 'Corralillo',
    '22.02': 'Mariel',
    '24.01': 'Bejucal',
    '22.01': 'Bahía Honda',
    '22.06': 'San Antonio de los Baños',
    '22.09': 'Artemisa',
    '22.03': 'Guanajay',
    '26.02': 'Quemado de Güines',
    '21.05': 'La Palma',
    '24.11': 'Quivicán',
    '24.08': 'Güines',
    '25.09': 'Unión de Reyes',
    '24.07': 'San Nicolás',
    '24.06': 'Nueva Paz',
    '25.05': 'Perico',
    '25.06': 'Jovellanos',
    '22.08': 'Alquízar',
    '25.07': 'Pedro Betancourt',
    '22.07': 'Güira de Melena',
    '22.10': 'Candelaria',
    '24.10': 'Batabanó',
    '25.13': 'Los Arabos',
    '24.09': 'Melena del Sur',
    '25.04': 'Colón',
    '21.04': 'Viñales',
    '22.11': 'San Cristóbal',
    '26.11': 'Santo Domingo',
    '26.10': 'Cifuentes',
    '25.11': 'Jagüey Grande',
    '21.06': 'Los Palacios',
    '25.12': 'Calimete',
    '21.03': 'Minas de Matahambre',
    '21.07': 'Consolación del Sur',
    '21.02': 'Mantua',
    '27.02': 'Rodas',
    '26.12': 'Ranchuelo',
    '27.04': 'Lajas',
    '25.10': 'Cienaga de Zapata',
    '26.09': 'Santa Clara',
    '21.08': 'Pinar del Río',
    '27.01': 'Aguada de Pasajeros',
    '26.13': 'Manicaragua',
    '21.10': 'San Juan y Martínez',
    '21.11': 'Guane',
    '27.03': 'Palmira',
    '27.05': 'Cruces',
    '27.06': 'Cumanayagua',
    '27.07': 'Cienfuegos',
    '27.08': 'Abreus',
    '21.09': 'San Luis',
    '21.01': 'Sandino',
    '40.01': 'Isla de la Juventud',
    '28.06': 'Trinidad',
    '26.04': 'Encrucijada',
    '29.01': 'Chambas',
    '26.05': 'Camajuaní',
    '26.06': 'Caibarién',
    '28.01': 'Yaguajay',
    '30.05': 'Nuevitas',
    '26.07': 'Remedios',
    '26.08': 'Placetas',
    '29.02': 'Morón',
    '28.04': 'Cabaiguán',
    '28.05': 'Fomento',
    '28.03': 'Taguasco',
    '29.06': 'Florencia',
    '28.02': 'Jatibonico',
    '29.05': 'Ciro Redondo',
    '29.03': 'Bolivia',
    '29.04': 'Primero de Enero',
    '28.07': 'Sancti Spíritus',
    '29.07': 'Majagua',
    '30.02': 'Esmeralda',
    '29.08': 'Ciego de Ávila',
    '30.03': 'Sierra de Cubitas',
    '29.10': 'Baraguá',
    '29.09': 'Venezuela',
    '30.04': 'Minas',
    '30.01': 'Carlos Manuel de Céspedes',
    '28.08': 'La Sierpe',
    '30.09': 'Florida',
    '30.08': 'Camagüey',
    '30.06': 'Guáimaro',
    '30.07': 'Sibanicú',
    '31.01': 'Manatí',
    '30.10': 'Vertientes',
    '30.11': 'Jimaguayú',
    '31.03': 'Jesús Menéndez',
    '31.02': 'Puerto Padre',
    '32.03': 'Banes',
    '30.13': 'Santa Cruz del Sur',
    '32.01': 'Gibara',
    '30.12': 'Najasa',
    '32.02': 'Rafael Freyre',
    '31.05': 'Las Tunas',
    '32.07': 'Calixto García',
    '32.06': 'Holguín',
    '31.06': 'Jobabo',
    '31.07': 'Colombia',
    '31.08': 'Amancio',
    '31.04': 'Majibacoa',
    '32.05': 'Báguanos',
    '32.04': 'Antilla',
    '32.11': 'Mayarí',
    '32.08': 'Cacocum',
    '33.01': 'Río Cauto',
    '32.14': 'Moa',
    '33.02': 'Cauto Cristo',
    '32.12': 'Frank País',
    '32.10': 'Cueto',
    '32.09': 'Urbano Noris',
    '32.13': 'Sagua de Tánamo',
    '34.04': 'Segundo Frente',
    '33.04': 'Bayamo',
    '33.03': 'Jiguaní',
    '35.09': 'Guantánamo',
    '34.02': 'Mella',
    '35.03': 'Yateras',
    '34.01': 'Contramaestre',
    '33.05': 'Yara',
    '34.03': 'San Luis',
    '34.07': 'Palma Soriano',
    '35.01': 'El Salvador',
    '33.06': 'Manzanillo',
    '34.05': 'Songo La Maya',
    '33.13': 'Guisa',
    '34.08': 'Tercer Frente',
    '33.12': 'Buey Arriba',
    '35.02': 'Manuel Tames',
    '33.11': 'Bartolomé Masó',
    '35.10': 'Niceto Pérez',
    '33.08': 'Media Luna',
    '34.06': 'Santiago de Cuba',
    '34.09': 'Guamá',
    '35.08': 'Caimanera',
    '35.07': 'San Antonio del Sur',
    '33.09': 'Niquero',
    '33.10': 'Pilón',
    '35.04': 'Baracoa',
    '35.06': 'Imías',
    '35.05': 'Maisí',
    '33.07': 'Campechuela'
}

def add_municipalities(apps, schema_editor):
    for code in municipality_codes:
        try:
            province = Province.objects.get(code=code.split('.')[0])
        except:
            province = None
        m = Municipality(province=province, code=code, name=municipality_codes[code])
        m.save()

class Migration(migrations.Migration):

    dependencies = [
        ('ads', '0002_fill_province_20200422_2155'),
    ]

    operations = [
        migrations.RunPython(add_municipalities),
    ]
